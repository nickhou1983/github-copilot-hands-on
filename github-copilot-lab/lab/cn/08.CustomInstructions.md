...
2. **创建copilot-instructions.md**
   ...
   在 VS Code Copilot Chat对话中选择Agent模式，输入 /create-instructions，文件将被创建到.github/copilot-instructions.md

   ![alt text](image-3.png)

   /create-instructions 是一个专门用来创建 instructions 文件的技能（skill），它会引导你提取之前对话中的偏好和规则，形成一条条具体的指令，并保存到一个 markdown 文件中。这个文件会被 Copilot 自动引用，成为你项目的一部分工作契约。
