...
> 💡 **提示**：`/create-skill` 命令由 skill-creator 技能提供。如果未出现在列表中，请确认该技能文件夹已正确放置在 `~/.copilot/skills/skill-creator/` 目录下
...
验证创建的 `frontend-review` Skill 能正确触发并执行前端页面检查

#### 4.2 操作步骤

1. **创建测试用 HTML 页面**

   在项目目录下创建一个包含常见问题的测试文件 `test-page.html`：
   ...
   <body>
       ...
       <div class="content">
           <div class="title" style="font-size: 24px; color: #999;">Welcome</div>
           <img src="banner.jpg">
           <div class="list">
               <div>Item 1</div>
               <div>Item 2</div>
   ...
   这个页面故意包含了以下问题：
   - ❌ 未使用语义化标签（`<div>` 替代 `<header>`, `<nav>`, `<main>`, `<footer>`）
   - ❌ `<img>` 缺少 `alt` 属性
   - ❌ `<img>` 缺少 `width` 和 `height` 属性
   - ❌ 表单 `<input>` 缺少关联的 `<label>`
   - ❌ 使用 `<div>` 模拟按钮，而非 `<button>`
   - ❌ 使用内联样式
   - ❌ `<html>` 缺少 `lang` 属性
   - ❌ 缺少 viewport meta 标签
   - ❌ 同步 JS 脚本放在 `<head>` 中阻塞渲染
   - ❌ 使用 `<div>` 模拟列表，而非 `<ul>`

2. **触发 Skill 执行检查**

   在 VS Code 中打开 Copilot Chat，选择 Agent 模式，将 `test-page.html` 添加到上下文中，输入以下提示词：
...
4. **验证检查结果**
   ...
   | 类别 | 预期发现 |
   |------|----------|
   | HTML 语义化 | `<div>` 应替换为 `<header>`, `<nav>`, `<main>`, `<footer>`, `<ul>` |
   | 可访问性 | `<img>` 缺少 `alt`、`<input>` 缺少 `<label>`、`<html>` 缺少 `lang` |
   | CSS 最佳实践 | 使用了内联样式 `style` 属性 |
   | 性能优化 | `<img>` 缺少宽高属性、同步脚本阻塞渲染 |
   | 响应式设计 | 缺少 viewport meta 标签 |
